...
| 创新点             | 技术实现                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| 三异构模型多数投票 | DeepSeek-R1（推理增强）+ Qwen（中文优化）+ GLM（通用），三模型独立判断后投票 |
...
| 单端口一体化部署   | 后端直接托管前端静态文件，无需额外Nginx                                      |
| 静默启动体验       | pythonw后台启动，无黑框，自动打开浏览器                                     |
...
③ 文本预处理
  ...
  ↓
④ 三模型并行分类 (SSE推送: 分类进度)
   ├─ DeepSeek  → {substantive/descriptive/non_env}
   ├─ Qwen      → {substantive/descriptive/non_env}
   ├─ GLM       → {substantive/descriptive/non_env}
...
| 模型     | 优势                         | 角色         |
| -------- | ---------------------------- | ------------ |
| DeepSeek | 理能力强，思维链深度分析     | 主推理模型   |
| Qwen     | 中文语料丰富，金融领域理解好 | 中文语义校准 |
| GLM      | 响应速度快，通用性好         | 快速验证模型 |
